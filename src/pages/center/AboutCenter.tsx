...
const AboutCenter = () => {
  ...
  const tabItems = {
    ...
    children: [
      { path: "staff", label: t("Markaz rahbariyati"), icon: FaUsers },
      { path: "departments", label: t("Markaz boʻlimlari"), icon: FaBuilding },
      { path: "structure", label: t("Markaz tuzilmasi"), icon: FaSitemap },
      { path: "vacancy", label: t("Boʻsh ish oʻrinlari"), icon: FaBriefcase },
      {
        path: "kiberxavfsizlik-klubi",
        label: t("Kiberxavfsizlik klubi"),
        icon: FaShieldAlt,
      },
      { path: "Hamkorlar", label: t("Hamkorlar"), icon: FaGlobe },
    ],
  };

  const activeChild = tabItems.children.find((child) =>
    location.pathname.includes(`/company/${child.path}`),
  );
                ...
                <div className="space-y-2">
                  {tabItems.children.map((item, index) => {
                    ...
                    return (
                      <motion.button
                        key={index}
                        onClick={() => navigate(`/company/${item.path}`)}
                        onMouseEnter={() => setHoveredTab(item.path)}
                        onMouseLeave={() => setHoveredTab(null)}
                        className={`
        relative flex w-full items-center gap-3 rounded-lg
        p-3 transition-all duration-200
        ${isActive ? "bg-[#036] text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200 dark:bg-gray-700 dark:text-white dark:hover:bg-gray-600"}
        ${isHovered ? "" : ""}
      `}
                        whileTap={{ scale: 0.98 }}
                      >
                        <div
                          className={`flex size-8 items-center justify-center rounded-full ${isActive ? "bg-white/20" : "bg-white dark:bg-gray-600"}`}
                        >
                          <Icon
