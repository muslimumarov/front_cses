...
const useGetEducationRecommendations = () => {
  const [page, setPage] = useState(1);
  const limit = 9;

  const { data, isLoading, error } = useGetAll<unknown>({
    url: ["recommendation"],
    queryKey: ["recommendations-education", page],
    params: { category: "EDUCATIONAL", page, limit },
  });
...
export default useGetEducationRecommendations;
